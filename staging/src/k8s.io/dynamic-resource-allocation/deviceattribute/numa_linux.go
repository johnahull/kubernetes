...
// numa_node). Additional elements are NUMA nodes equidistant to the device
// per the ACPI SLIT distance matrix, filtered to the same socket.
//
// Falls back to a single-element list if SLIT distances are unavailable.
...
// already knows its NUMA node. This is suitable for CPU and memory devices.
// The first element is the physical NUMA node. Additional elements are
// equidistant NUMA nodes per SLIT, filtered to the same socket.
func GetNUMANodeList(numaNode int, mods ...MachineModifier) DeviceAttribute {
	...
	return makeNUMANodeListAttribute(mc, numaNode)
}
...
func makeNUMANodeListAttribute(mc machine, physicalNode int) DeviceAttribute {
	...
	// Physical node first, then remaining equidistant nodes
