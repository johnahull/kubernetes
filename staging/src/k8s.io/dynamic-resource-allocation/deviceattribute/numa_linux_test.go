...
func TestGetNUMANodeListByPCIBusID(t *testing.T) {
	pciBusID := "0000:02:00.0"
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			got, err := GetNUMANodeListByPCIBusID(test.address, WithFSFromRoot(testMachinePath))
			if test.expectsError {
				if err == nil {
					t.Errorf("Expected error but got none")
					return
...
func TestGetNUMANodeList(t *testing.T) {
	tests := map[string]struct {
		testMachineSetup func(t *testing.T, testRootPath string)
		numaNode         int
		expectedValues   []int64
	}{
		"single NUMA node - no SLIT": {
			numaNode:       0,
			expectedValues: []int64{0},
		},
		"negative NUMA node": {
		...
		"NUMA node with equidistant peers": {
			testMachineSetup: func(t *testing.T, testRootPath string) {
				setupNUMADistance(t, testRootPath, 2, "32 32 10 12 12 12 32 32")
				setupNUMANodeSocket(t, testRootPath, 2, "48-71", "0")
				setupNUMANodeSocket(t, testRootPath, 3, "72-95", "0")
				setupNUMANodeSocket(t, testRootPath, 4, "96-119", "0")
				setupNUMANodeSocket(t, testRootPath, 5, "120-143", "0")
			},
			numaNode:       2,
			expectedValues: []int64{2, 3, 4, 5},
		},
	}
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			got := GetNUMANodeList(test.numaNode, WithFSFromRoot(testMachinePath))
			expected := DeviceAttribute{
				Name:  StandardDeviceAttributeNUMANode,
				Value: resourceapi.DeviceAttribute{IntValues: test.expectedValues},
			}
...
func TestGetNUMANodeForCPU(t *testing.T) {
	tests := map[string]struct {
		...
		"CPU 0 on NUMA node 0": {
			testMachineSetup: func(t *testing.T, testRootPath string) {
				setupNUMANodeCPUList(t, testRootPath, 0, "0-3")
			},
			cpuID:        0,
			expectedNode: 0,
		},
		"CPU 5 on NUMA node 1": {
			testMachineSetup: func(t *testing.T, testRootPath string) {
				setupNUMANodeCPUList(t, testRootPath, 0, "0-3")
				setupNUMANodeCPUList(t, testRootPath, 1, "4-7")
			},
			cpuID:        5,
			expectedNode: 1,
		},
		"CPU in range with gaps": {
			testMachineSetup: func(t *testing.T, testRootPath string) {
				setupNUMANodeCPUList(t, testRootPath, 0, "0-3,8-11")
				setupNUMANodeCPUList(t, testRootPath, 1, "4-7,12-15")
			},
			cpuID:        9,
			expectedNode: 0,
		},
		"CPU on high NUMA node": {
			testMachineSetup: func(t *testing.T, testRootPath string) {
				setupNUMANodeCPUList(t, testRootPath, 0, "0-31")
				setupNUMANodeCPUList(t, testRootPath, 1, "32-63")
				setupNUMANodeCPUList(t, testRootPath, 2, "64-95")
				setupNUMANodeCPUList(t, testRootPath, 3, "96-127")
			},
			cpuID:        100,
			expectedNode: 3,
		},
...
func setupPCINUMANode(t *testing.T, testRootPath string, pciBusID string, numaNode string) {
	...
	writeTestFile(t, numaNodeFile, numaNode+"\n")
}
...
func setupNUMADistance(t *testing.T, testRootPath string, nodeNum int, distances string) {
	...
	writeTestFile(t, distFile, distances+"\n")
}

func setupNUMANodeCPUList(t *testing.T, testRootPath string, nodeNum int, cpulist string) {
	t.Helper()
...
func setupNUMANodeSocket(t *testing.T, testRootPath string, nodeNum int, cpulist string, socketID string) {
	...
	if len(cpus) > 0 {
		...
		writeTestFile(t, pkgFile, socketID+"\n")
	}
}
